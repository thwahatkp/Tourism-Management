...
-->
  ...
  <table class="table " id="viewGallery">
    <thead>
      <tr>
...
while($row=$result=mysqli_fetch_assoc($query)){
  ?>
          ...
          <td><img style="width: 190px;max-height: 200px;" src="../gallery/<?= $result["Src"]?>" alt="<?= $row["Place-name"] ?>"></td>
          <td class="text-center"><?= $row["Place-name"] ?></td>  
          <td class="text-center">
          <button class="btn btn-success" style="height:40px" onclick="galleryInsertForm()">Insert</button>
          <button class="btn btn-danger" style="height:40px" onclick="galleryDelete('<?=$row['Id']?>','<?=$row['Src']?>')">Delete</button></td>
        </tr>
